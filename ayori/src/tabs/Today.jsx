...
import { FULL_DAYS, DAYS, HABITS, WORKOUT_TYPES } from "../lib/constants";
import { sumLog, getSuggestion } from "../lib/helpers";
import { Arc, Bar, BigNum, GlossySphere, GhostBtn, SolidBtn, Over } from "../components/ui";

export default function Dashboard({ t, today, dayIdx, todayLog, setTodayLog, targets, readiness, setReadiness, schedule, weekHabits, setActiveTab, socialFlag, mealLibrary }) {
  ...
  return (
    <div style={{ padding:"0 24px 48px" }}>
      ...
      {/* Calorie ring — centered + large */}
      <div style={{ padding:"32px 0 0", display:"flex", flexDirection:"column", alignItems:"center" }}>
        <Arc value={totals.cal} max={targets.total} size={160} sw={6} color={t.ring} label="kcal" sub={targets.total} t={t}/>
        <div style={{ fontSize:11, color:remaining.cal<0?t.over:t.textDim, letterSpacing:1, marginTop:16, marginBottom:28 }}>
      ...
      <div style={{ padding:"20px 0", borderBottom:`1px solid ${t.border}`, display:"flex", alignItems:"center", gap:20 }}>
        ...
        <div style={{ flex:1, display:"flex", gap:8, flexWrap:"wrap" }}>
          ...
          <input type="number" value={readinessInput} onChange={e=>{setReadinessInput(e.target.value);const n=+e.target.value;if(n>=40&&n<=100)setReadiness(n);}} placeholder="or score" min={40} max={100}
            style={{ width:60, padding:"6px 8px", border:"none", borderBottom:`1px solid ${t.border}`, background:"transparent", color:t.text, fontSize:11, fontFamily:"inherit", outline:"none" }}/>
        </div>
      </div>
          ...
          {HABITS.map(h=>{
            ...
            return (
              <motion.button key={h.key} onClick={()=>setActiveTab("habits")}
                ...
                {streak>0&&<div style={{ fontSize:8, color:col, letterSpacing:1.5, textTransform:"uppercase" }}>{streak}d</div>}
              </motion.button>
            );
          })}
        </div>
      ...
      <div style={{ padding:"24px 0", borderBottom:`1px solid ${t.border}`, display:"flex", flexDirection:"column", gap:10 }}>
        <SolidBtn t={t} onClick={()=>setActiveTab("coach")} style={{ width:"100%", textAlign:"center" }}>Ask Coach</SolidBtn>
        <button onClick={()=>setActiveTab("log")} style={{ background:"transparent", border:"none", color:t.textDim, fontSize:10, letterSpacing:3, textTransform:"uppercase", cursor:"pointer", fontFamily:"inherit", padding:"6px 0", textAlign:"center", minHeight:44 }}>
          + Log a Meal
        </button>
      </div>

      {/* Empty state nudge */}
      {todayLog.length===0&&readiness===null&&(
        <div style={{ padding:"32px 0", textAlign:"center" }}>
          <div style={{ fontSize:13, fontWeight:300, color:t.textDim, lineHeight:1.9, maxWidth:260, margin:"0 auto" }}>
            Start by setting your Oura score above, then log your first meal to see your macros fill in.
          </div>
        </div>
      )}

      {/* Logged Today with running calorie subtotal */}
      {todayLog.length>0&&(
                ...
                return (
                  <div key={i} onClick={()=>{setEditingIdx(i);setEditForm({...m});}} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"13px 0", borderBottom:`1px solid ${t.border}`, cursor:"pointer" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:3 }}>
                      ...
                      <div style={{ fontSize:9, color:t.textDim, letterSpacing:0.5 }}>= {runningCal}</div>
                    </div>
                  </div>
                );
              });
